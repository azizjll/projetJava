...
public class AnnonceController {
        ...
        try (Connection connection = ConnectionManager.getConnection()) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(query);
                ...
                while (resultSet.next()) {
                    ...
                    Annonce annonce = new Annonce(id, titre, description, (Timestamp) date);
    ...
    private void addAnnonce(ActionEvent event) throws MessagingException {
        ...
        if (titre.isEmpty() || titre.length() > 100 || description.isEmpty()|| description.length() > 500) {
            ...
            if (addAnnouncementToDatabase(titre, description,user_id)) {
   ...
   /* @FXML
    ...
    private boolean addAnnouncementToDatabase(String titre, String description,int user_id) {
        String query = "INSERT INTO annonce (titre, description, date,user_id ) VALUES (?, ?, ?,?)";
        try (Connection connection = ConnectionManager.getConnection();
             PreparedStatement statement = connection.prepareStatement(query)) {
